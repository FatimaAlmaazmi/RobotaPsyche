class Orca {
  ...
  void seekPod(ArrayList<Orca> orcas) {
    for (Orca pod : orcas) {
      if (isSad == false) {
        PVector desired = PVector.sub(pod.location, location);
        desired.normalize();
        desired.mult(maxSpeed);
        PVector steer = PVector.sub(desired, velocity);
        steer.limit(maxForce);
  ...
  void helpSad(ArrayList<Orca> orcas) {
    ...
    for (Orca other : orcas) {
      ...
      boolean otherIsSad = other.isSad; // to make sure other is sad.

      if (otherIsSad) { // if other is indeed sad do all this:
        // println("seeking sad robot");
  ...
  void helpDorsal(ArrayList<Orca> orcas) {
    ...
    for (Orca other : orcas) {
      ...
      boolean otherIsDorsal = other.getIsDorsal();

      if (otherIsDorsal) {
        //  println("seeking dorsal");
        //  println();
        sum = seek(other.location);
